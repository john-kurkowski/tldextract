...
# Python 3: Get __version__ and __doc__ from other files without tripping over
# Python 3 incompatible code in tldextract.py which is yet to be translated by
# use_2to3
exec(compile(open("tldextract/version.py").read(), "tldextract/version.py", 'exec'))
exec(compile(open("tldextract/docstring.py").read(), "tldextract/docstring.py", 'exec'))

# I don't want to learn reStructuredText right now, so strip Markdown links
# that make pip barf.
long_description_markdown = __doc__
